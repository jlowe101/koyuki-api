...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($json_data['cookie'])) {
        ...
        // Added Accept and Expect headers to prevent the API from dropping the connection
        ...
        // Safely extract headers (like Retry-After) without triggering array indexing errors
        ...
        // Fully disable SSL checks in case the API's SSL certificate is incomplete
        ...
        if ($http_code === 200 && $response) {
            ...
            if (is_array($parsed)) {
                // Mimic the frontend's intelligent success detection
                ...
                if ($is_success && isset($pdo)) {
                    try {
                        // Check if cookie is already stored to prevent spamming duplicates
        ...
        // Forward the Retry-After header to the frontend if rate limited (HTTP 429)
        ...
        if ($response === false) {
            $http_code = 500;
            $response = json_encode(['error' => 'cURL connection failed: ' . $curl_error]);
        }
...
<head>
    ...
    <style>
        ...
        /* Gradient Premium Button - Cold Blue */
        .btn-premium { 
...
<nav class="fixed top-0 left-0 w-full z-50 bg-[#020613]/80 backdrop-blur-md border-b border-[rgba(0,229,255,0.2)] shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
    <div class="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-20">
            ...
            <div class="flex items-center gap-2 sm:gap-3">
                ...
                <span class="rye-font text-xl sm:text-2xl tracking-wide glow-text mt-1">Koyuki NF Checker</span>
            </div>
            <!-- Right: Links (Desktop) -->
    ...
    <div id="mobileMenu" class="hidden sm:hidden bg-[#020813] border-t border-[rgba(0,229,255,0.1)] py-4 px-6 shadow-[0_10px_30px_rgba(0,0,0,0.8)]">
        <div class="flex flex-col gap-4">
            ...
            <a href="contact.php" class="text-sm font-bold text-slate-300 hover:text-[#00e5ff] transition-colors uppercase tracking-wider flex items-center justify-center pt-2">
                Contact
            </a>
        </div>
    </div>
        ...
        <div class="col-12 col-md-11 col-lg-10">
            ...
            <div class="premium-card p-4 sm:p-5 mb-5 relative overflow-hidden">
                ...
                <div class="relative z-10">
                    <!-- UNIFIED CHECK SECTION (Handles both Single & Bulk natively) -->
            ...
            <div class="premium-card p-4 sm:p-5" id="resultsCard" style="display: none;">
                ...
                <!-- LIVE STATS DASHBOARD (Mobile Wrapped) -->
...
<script>
    // Store live accounts for downloading
    let bulkLiveAccounts = [];

    // Download Function for all successful hits
    ...
    // Copy Details Function utilizing SweetAlert2 for notifications
    function copyCardDetails(encodedStr) {
        try {
            ...
            Swal.fire({
                ...
                background: '#020613',
                color: '#00e5ff',
                customClass: { popup: 'border border-[rgba(0,229,255,0.3)] font-mono text-sm sm:text-base' }
            });
        } catch (err) {
    ...
    // The Smart Parser (Extracts Netscape & JSON format cookies)
    ...
    function createResultElement(data, httpStatus, indexNum, originalCookie) {
        ...
        const isSuccess = ['accepted', 'working', 'live', 'valid', 'success'].includes(verdictStr) || 
                          signalStr.includes('active auth path') || 
                          signalStr.includes('membership found') ||
                          signalStr.includes('confirmed');

        if (isSuccess) {
            const details = data.accountDetails || {};
            const links = data.watchLinks || {};
            
            let planStr = details.plan || 'N/A';
            let isPremium = planStr.toLowerCase().includes('premium');
            let planColor = isPremium ? '#34d399' : '#00e5ff';
            
            // Format Profiles into Badges
            let rawProfiles = details.profiles || data.profiles || 'N/A';
            let profilesHtml = '';
            
            if (rawProfiles === 'N/A') {
                profilesHtml = `<span class="text-slate-500 text-sm">N/A</span>`;
            } else {
                let profArray = Array.isArray(rawProfiles) ? rawProfiles : String(rawProfiles).split(',');
                profilesHtml = profArray.map(p => 
                    `<span class="inline-block bg-[rgba(0,229,255,0.1)] text-[#00e5ff] px-3 py-1.5 rounded-full text-[10px] sm:text-xs font-semibold border border-[rgba(0,229,255,0.3)] mr-2 mb-2 shadow-[0_0_8px_rgba(0,229,255,0.15)]"><i class="fas fa-user-circle mr-1.5 opacity-80"></i>${p.trim()}</span>`
                ).join('');
            }
            
            let directLink = links.pc || '#';
            let mobileLink = links.mobile || '#';
            let tvLink = links.tv || '#';
            let displayedEmail = details.email || 'Valid Account / Hidden Email';

            // Generate Copy Layout 
            ...
            <div class="result-item">
                <div class="d-flex flex-col sm:flex-row justify-content-between align-items-start mb-3 border-b border-[rgba(0,229,255,0.15)] sm:border-none pb-3 sm:pb-0 gap-2 sm:gap-0">
                    <div>
                        <span class="status-badge status-success mr-2"><i class="fas fa-check-circle mr-1"></i> LIVE</span>
                        <span class="status-badge bg-slate-800 text-slate-300 border border-slate-700"><i class="fas fa-fingerprint mr-1"></i> NODE_${indexNum}</span>
                    </div>
                ...
                <div class="row g-3 mb-4">
                    <div class="col-sm-4 col-6">
                        ...
                        <div class="detail-value" style="color: ${planColor}; font-weight: 700;">${planStr} <i class="${isPremium ? 'fas fa-crown ml-1' : ''}"></i></div>
                    </div>
                    <div class="col-sm-4 col-6">
                    ...
                    <div class="col-sm-4 col-6">
                        ...
                        <div class="detail-value">${details.paymentMethod || 'N/A'}</div>
                    </div>
                    <div class="col-sm-4 col-6">
                        <div class="detail-label">Phone Linked</div>
                        <div class="detail-value">${details.phone || 'N/A'}</div>
                    </div>
                    <!-- Expanded API Signal width -->
                    <div class="col-sm-8 col-12">
                        <div class="detail-label">API Signal</div>
                        <div class="detail-value text-[#00e5ff] truncate" title="${data.matchedSignal || 'Active member'}">${data.matchedSignal || 'Active member'}</div>
                    </div>
                    <!-- PROFILES FULL WIDTH ROW WITH BADGES -->
                    <div class="col-12 mt-2 pt-3 border-t border-[rgba(0,229,255,0.15)]">
                        <div class="detail-label mb-2">Active Profiles</div>
                        <div class="mt-1 flex flex-wrap">${profilesHtml}</div>
                    </div>
                </div>
            ...
            <div class="result-item d-flex align-items-center">
                <span class="status-badge status-warning mr-3"><i class="fas fa-exclamation-triangle mr-1"></i> RATE LIMITED</span>
                <div class="ml-2">
                    <div class="text-white font-semibold">${data.error || 'Please slow down.'}</div>
                    <div class="text-muted small">Code: ${data.code || 'unknown'}</div>
                </div>
            </div>`;
            
        } else {
            let failReason = data.matchedSignal || data.accountStatus || data.error || 'Invalid Cookie Session';
            let verdictText = data.verdict || 'FAILED';
            
            resultHtml = `
            <div class="result-item d-flex align-items-center">
                <span class="status-badge status-error mr-3"><i class="fas fa-times-circle mr-1"></i> ${verdictText}</span>
                <div class="ml-2">
                    <div class="text-slate-300 font-medium break-all sm:break-normal">${failReason}</div>
                    <div class="text-slate-500 small">Node_${indexNum} execution rejected</div>
                </div>
            </div>`;
        }
        return resultHtml;
    ...
    async function processBulk() {
        ...
        // Even if the parser fails to find a strict format, try checking the raw string anyway.
        ...
        // Show Stats Board and Reset Counters
        $('#bulkStats').css('display', 'flex').addClass('fade-in');
        let liveCount = 0;
        let deadCount = 0;
        let limitCount = 0;
        
        $('#statActive').text(liveCount);
        ...
        for (let i = 0; i < cookiesToProcess.length; i++) {
            ...
            let dynamicWait = 2000; // Default wait time
            
            try {
                const response = await fetch(apiUrl, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ cookie: cookiesToProcess[i] })
                });

                const retryAfter = response.headers.get('Retry-After');
                if (retryAfter) {
                    dynamicWait = (parseInt(retryAfter, 10) * 1000) + 1000;
                }
                ...
                try {
                    ...
                    throw new Error(rawResponseText.substring(0, 100) || "Invalid API Response.");
                }
                ...
                if (isSuccess) {
                    ...
                    // Add details to the bulk download array
                    const details = data.accountDetails || {};
                    const links = data.watchLinks || {};
                    let planStr = details.plan || 'N/A';
                    let rawProfiles = details.profiles || data.profiles || 'N/A';
                ...
                // Add to Carousel and pass the original cookie to the generator
                totalCards++;
                ...
                let isActiveClass = (totalCards === 1) ? 'active' : '';
                $('#bulkResultsList').append(`
                    <div class="carousel-card ${isActiveClass}">
                        <div class="result-item d-flex align-items-center"><span class="status-badge status-error mr-3">SYS_ERR</span><span class="text-slate-400 break-all sm:break-normal text-xs sm:text-sm">${error.message}</span></div>
                    </div>
                `);
                updateCarouselUI();
            }
        ...
        // Show download button if we had hits
        if (bulkLiveAccounts.length > 0) {
            $('#downloadWrapper').fadeIn();
        }
    }
